function init_multiline_plot(element_id, uid_data, actions_data) {
    ...
    dots.enter().append('circle')
        ...
        .on("mouseover", function(d) {
            div.transition()
                .duration(200)
                .style("opacity", .9);
            div.html(formatTime(d.date) + "<br/>Users: "  + parseInt(d.value * 1000000))
    ...
    var data1 = Array();
    actions_data.forEach(function(d) {
    i = 0
    for (property in d) {
        data1[i] = {date:parseDate(property),value:(+d[property]/1000000)}
        i++;
    }
    });
    ...
    svg.append("path")
      .datum(data1)
      .attr("class", "actions_line")
      .attr("d", line);

    dataDotsGroup = svg.append('g');
    var dots1 = dataDotsGroup.selectAll('.data-point').data(data1);
    var div = d3.select("body").append("div")
        .attr("class", "tooltip")
        .style("opacity", 0);
    var formatTime = d3.time.format("%e %b %Y");
    dots1.enter().append('circle')
        ...
        .on("mouseover", function(d) {
            div.transition()
                .duration(200)
                .style("opacity", .9);
            div.html(formatTime(d.date) + "<br/>Actions: "  + parseInt(d.value * 1000000))
                .style("left", (d3.event.pageX - 60) + "px")
                .style("top", (d3.event.pageY - 50) + "px");
            })
        .on("mouseout", function(d) {
    ...
    dataCirclesGroup1 = svg.append('g');
    var circles1 = dataCirclesGroup1.selectAll('.data-point').data(data1);
